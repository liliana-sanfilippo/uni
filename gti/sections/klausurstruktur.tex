...
\begin{itemize}
    ...
    \item Das Pumping-Lemma ist \textbf{nur notwendig}, nicht hinreichend. Es beweist Nicht-Regularität,
...
\begin{itemize}
    ...
    \begin{itemize}
        \item $G=(\{S_0,S_1\},\{a,b,c\},\{S_0::=bS_0\mid aS_1\mid bS_1\mid a,\ S_1::=aS_0\mid b\},S_0)$ \txc{(PK2021)}
        \item $G_1=(\{S_0,S_1\},\{a,b\},\{S_0::=aS_1\mid bS_0\mid bS_1\mid a,\ S_1::=aS_0\mid b\},S_0)$ \txc{(PK2023)}
        \item $G:=(\{S_0,S_1,S_2,S_3\},\{a,b,c\},R,S_0)$ mit\\
        $R:=\{S_0::=cS_0\mid c\mid aS_1\mid bS_1,\ S_1::=cS_1\mid aS_2\mid bS_2,\ S_2::=cS_2\mid aS_3\mid bS_3\mid a,\ S_3::=cS_3\mid c\}$ \txc{(NK2023)}
        \item $G:=(\{S_0,S_1,S_2\},\{a,b,c\},R,S_0)$ mit\\
        $R:=\{S_0::=aS_0\mid cS_0\mid aS_1,\ S_1::=cS_1\mid aS_2\mid bS_2,\ S_2::=aS_2\mid bS_2\mid a\mid b\}$ \txc{(PK2025)}
    \end{itemize}
    ...
    \begin{itemize}
        \item $G=(\{A_0,A_1,B_0,B_1\},\{a,b\},\{A_0::=aA_1\mid B_0,\ A_1::=aA_0\mid B_1\mid a,\ B_0::=bB_1\mid\eps,\ B_1::=bB_0\},A_0)$ \txc{(HK2019)}
        \item $G=(\{A_0,A_1,B_0,B_1\},\{a,b\},\{A_0::=aA_1\mid B_0,\ A_1::=aA_0\mid B_1\mid a,\ B_0::=bB_1\mid\eps,\ B_1::=bB_0\},A_0)$ \txc{(NK2019)}
    \end{itemize}
    \item Geben Sie einen
\end{itemize}

\subsubsection{Potenzmengenkonstruktion NEA $\to$ DEA.}%\txc{2017H, 2017N, 2019H, 2019N, 2021, 2023P, 2023Z, 2025}
Fast jede Klausur... Mal als Diagramm, mal als Übergangstabelle. Müllzustand $\emptyset$ mitnehmen. Zustände = Teilmengen
...
\begin{itemize}
    ...
    \end{itemize}
    \item Gegeben ein endlicher Automat $A=(\Phi,\Sigma,\Delta,q_0,F)$. Betrachte $A_C=(\Phi,\Sigma,\Delta,q_0,\Phi\setminus F)$.
    Unter welcher Annahme gilt $L(A_C)=L(A)^C$? Warum ist diese Annahme notwendig? \txc{(PK2023)}
    \item Gegeben ein DEA $A=(\Phi,\Sigma,\delta,q_0,F)$. Konstruieren Sie einen DEA (in Tupelnotation),
    der genau das Komplement von $L(A)$ akzeptiert. Begründen Sie. \txc{(HK2019)} \txc{(NK2019)}
\end{itemize}
...
\begin{itemize}
    ...
    \item Konstruieren Sie einen NEA $A_*$ mit $L(A_*)=L(A)^*$ (in Tupelnotation). Erläutern Sie, wie $A_*$
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        $R:=\{S_0'::=aS_1'\mid bS_0'\mid cS_2'\mid a,\ S_1'::=aS_0'\mid bS_0'\mid cS_2'\mid b,\ S_2'::=aS_3',\ S_3'::=aS_2'\mid bS_2'\}$ \txc{(PK2023)}
    \end{itemize}
    ...
    \begin{itemize}
        \item
        $G'=(\{S_0',S_1',S_2'\},\{a,b\},\{S_0'::=aS_1'\mid aS_2'\mid bS_0'\mid a,\ S_1'::=aS_0'\mid bS_2'\mid b,\ S_2'::=bS_1'\},S_0')$ \txc{(NK2023)}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        $R:=\{S_0'::=bS_0'\mid aS_1'\mid bS_1'\mid aS_2'\mid S_3',\ S_1'::=aS_0'\mid b,\ S_2'::=aS_1'\mid b,\ S_3'::=S_3'\mid\eps\}$.\\
...
Schon lange her...vermutlich nicht wieder?
\begin{itemize}
    \item Betrachten Sie den NEA auf der linken Seite. Vervollständigen Sie gemäß der Konstruktion aus
    der Vorlesung den deterministischen Ersatzakzeptor (rechts vorgegebene Zustände).
    \begin{itemize}
        \item NEA \% Diagramm:\ $q_0\xrightarrow{a}q_0,\ q_0\xrightarrow{b}q_1,\ q_1\xrightarrow{a}q_1,\ q_1\xrightarrow{\eps}q_0$;
        vorgegebene DEA-Zustände $\{q_1\},\{q_0\},\{q_0,q_1\}$  \txc{(HK2019)}
        \item NEA \% Diagramm:\ $q_0\xrightarrow{a}q_0,\ q_0\xrightarrow{b}q_1,\ q_1\xrightarrow{b}q_2,\ q_2\xrightarrow{a}q_2,\ q_2\xrightarrow{\eps}q_0$;
...
\begin{itemize}
    \item Ist die Grammatik eindeutig? Begründen Sie.
    \begin{itemize}
        \item  $G=(\{S\},\{a,b\},\{S::=aSSa\mid b\mid a\},S)$ \txc{(HK2017)}
        \item $G=(\{S,B\},\{a,b\},\{S::=aBSa\mid\eps,\ B::=bS\mid Sb\},S)$ \txc{(NK2017)}
        \item  $G=(\{S,B\},\{a,b,c\},\{S::=aSc\mid B,\ B::=bB\mid\eps\},S)$ \txc{(HK2019)}
        \item $G=(\{S,B\},\{a,b,c\},\{S::=aSc\mid B,\ B::=bB\mid\eps\},S)$ \txc{(NK2019)}
        \item  $G=(\{S,B\},\{a,b,c\},\{S::=aScB\mid B,\ B::=bB\mid b\},S)$ \txc{(PK2021)}
        \item $G=(\{S\},\{a,b,c\},\{S::=bSSb\mid c\mid b\},S)$ \txc{(PK2025)}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item $G=(\{S,B\},\{a,b,c\},\{S::=aSc\mid B,\ B::=bB\mid\eps\},S)$ \txc{(NK2019)}
        \item  $G=(\{S,B\},\{a,b,c\},\{S::=aScB\mid B,\ B::=bB\mid b\},S)$ \txc{(PK2021)}
        \item  $G=(\{S\},\{(,a,)\},\{S::=(SS)\mid aS\mid a\},S)$ \txc{(PK2023)}
        \item $G=(\{S\},\{a,b\},\{S::=aSSa\mid a\mid b\},S)$ \txc{(NK2023)}
        \item $G=(\{S\},\{a,b,c\},\{S::=bSSb\mid c\mid b\},S)$ \txc{(PK2025)}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item  $w=(a(aaa))$ für $G=(\{S\},\{(,a,)\},\{S::=(SS)\mid aS\mid a\},S)$ \txc{(PK2023)}
        \item $G_1=(\{A,B,C\},\{a,b,c\},\{A::=AB\mid AC\mid BC\mid a,\ B::=BB\mid BC\mid b,\ C::=c\},A)$; prüfe $\underline{acb}\in L(G_1)$ \txc{(NK2023)}
        \item $G_{\mathrm{CNF}}=(\{A,B,C\},\{a,b,c\},\{A::=AB\mid BC\mid a,\ B::=BB\mid b,\ C::=AC\mid CC\mid c\},B)$;\\
...
Regeln $a\,q_0\,b::=\overline{w}\,q_1$ graphisch als $(a,b)/\overline{w}$.
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item  Regelschablonen $(\#,a)/\_,\ (\#,b)/\_,\ (a,a)/\_,\ (a,b)/\_,\ (a,c)/\_$ und $\#/\eps$ nach $f$;
...
\begin{mytable}
    \begin{tabular}{@{}p{0.60\textwidth} c p{0.30\textwidth}@{}}
        ...
        $\exists$ Paar kontextfreier $L_1,L_2$ mit $L_1\cup L_2$ kontextfrei. & W & CFL sogar immer unter $\cup$
        abgeschl.\\
        $\exists$ Paar kontextfreier $L_1,L_2$ mit $L_1\cap L_2$ kontextfrei. & W & ``es gibt ein Paar'' (z.\,B.\
        ...
        Für kontextfreie Sprachen ist das Wortproblem nicht entscheidbar. & F & CYK entscheidet es ($\mathcal O(n^3)$).\\
...
\begin{mytable}
    \begin{tabular}{@{}p{0.60\textwidth} c p{0.30\textwidth}@{}}
        ...
        $A$ NP-hart $\iff A\le_p\text{ILP}\wedge\text{ILP}\le_p A$. & F & beschreibt NP-vollständig, nicht NP-hart.\\
        ...
        $\{1,\dots,5\}^*\to\nz,\ (n_1,\dots,n_t)\mapsto\sum n_i 10^i$ ist Gödelisierung. & W\,(?) & Ziffern $<10$ $\Rightarrow$ injektiv; def.-abhängig (nicht surjektiv).\\
        $\N^*\to\N,\ (n_1,\dots,n_t)\mapsto\sum n_i 10^i$ ist Gödelisierung. & F & unbeschr.\ $n_i$ $\Rightarrow$
        ...
        TM mit 42 Zuständen simulierbar durch TM mit wenigen Zuständen. & W\,(?) & Zustands-/Zeichen-Tradeoff;
        Skript prüfen.\\
        Universelle TM mit einem Zustand. & (?) & umstritten...im Skript / Tutorium klären.\\
        \hline
    \end{tabular}
\end{mytable}