...
The unit cost edit distance simply counts certain differences between strings.
...
The following is called the \emph{transversion/transition cost} on the DNA alphabet:
\[
    \begin{array}{c|cccc}
...
\begin{notte}
    Note, however, that in a distance model a match has always zero cost,
    $\mathrm{cost}\!\left(\begin{smallmatrix}a\\a\end{smallmatrix}\right)=0$ for all $a\in\Sigma$.
    This
...
\defin{sensible_score_function}
The concept of a metric is useful because it embodies the essential properties of our intuition
about distances.
...
Similarity scores, instead, where matches are scored positively, while mismatches and
...
\begin{defi}
    A \emph{sensible score function} for columns in a pairwise alignment is a function
...
The first and second conditions impose symmetry and state that the similarity between $a$ and itself
...
Now it is time to discuss how we can define similarity scores for biological sequences from an
...
\defin{frequency_vector}

Let $\pi = (\pi_a)_{a\in\Sigma}$ be the frequency vector, which means that $\pi_a\ge 0$ for all
...
Now assume that we can track the fate of individual amino acids over a fixed evolutionary time
period $t$.
...
We usually demand that similarity functions and hence score matrices are symmetric.
In some cases,
...
The match/mismatch score in all our alignment algorithms depends on a (sensible) similarity function
...
\fbox{\parbox[c][3.2cm][c]{0.7\textwidth}{\centering\itshape
[Figure 4.1: Bipartite scoring scheme for detection of homologous transmembrane proteins
(Müller et al., 2001). Rows = query sequence positions: in TM helices use SLIM, elsewhere use
BLOSUM.]\\ \normalfont\scriptsize(original figure -- insert image here)}}
