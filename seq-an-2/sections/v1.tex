...
\section{The q-gram Distance}\label{sec:the-q-gram-distance}
...
Using lexicographic order (AA, AB, BA, BB) among the q-grams, we have $p_2(x) = (1, 1, 1, 0)$, $p_2(y) = (0, 2, 1, 0)$ and $p_2(z) = (1, 1, 1, 0)$.
...
If the q-gram profile of a string $x \in \sum^m$ is \gls{dense} (i.e., if it does not contain mostly zeros), it
makes sense to implement it as an array $p[0 \cdots \sum^q - 1]$, where $p[i]$ counts the number of occurrences of the
i-th q-gram in some arbitrary but fixed (e.g.\ lexicographic) order.
