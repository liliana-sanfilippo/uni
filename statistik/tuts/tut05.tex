...
\[
    A = \bigl\{(x,y)\in\R^2 : 0 \leq x,y \leq \frac{1}{2}\bigr\}
    \cup
    \bigl\{(x,y)\in\R^2 : \frac{1}{2} < x,y \leq 1\bigr\}.
\]
...
\begin{auf}[Zeigen Sie, dass sowohl $X$ als auch $Y$ uniform verteilt auf $[0,1]$ ist.]
    NOCH KEINE LÖSUNG % TODO NOCH KEINE LÖSUNG
\end{auf}

\begin{auf}[ Können Sie zwei Mengen $C, D \subseteq \R$ finden derart, dass die Ereignisse
