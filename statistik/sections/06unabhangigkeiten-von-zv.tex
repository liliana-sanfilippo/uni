...
\begin{bsp}
    ...
    \begin{itemize}
        \item Alex notiert den zweifachen Münzwurf \\ $\zvx : \eraum \rightarrow \eraum$ mit $\zvx = (\zvx_1, \zvx_2)$ wobei
    ...
    \end{table}
    Offensichtlich sind die Randverteilungen von $\zvx$ und von $Y$ gleich, während die gemeinsame Verteilung
    von $Y_1$ und $Y_2$ ist nicht dieselbe ist wie jene von $\zvx_1$ und $\zvx_2$.
\end{bsp}
...
\begin{bsp}
    ...
    Sei $(\zvx, Y)$ ein zufälliger Punkt in $\rz$ (uniforme Verteilung)
    \begin{figure}[H]
        \includegraphics[width=0.4\linewidth]{./images/viereck}\label{fig:figure2}
    \end{figure}
    $(\zvx, Y)$ hat (gem.) Dichte $f(x, y) = \chi_R (x, y) \frac{1}{a \cdot b} \quad \forall (x, y) \in \rz^2$
    Randdichten:
    \begin{align*}
    ...
    schlauer:
    \[
        f(x, y) = \chi_{\rz} (x, y) \frac{1}{a \cot b} = \frac{1}{a} \underbrace{\chi_{[0, a]}(x) }_{\text{Randdichte von } $\zvx$} \cdot \frac{1}{b} \underbrace{\chi_{[0, b]}(y) }_{\text{Randdichte von } Y}
    \]
...
\begin{bsp}
    ...
    \end{figure}
    Sei $(\zvx, Y)$ ein zufälliger Punkt \txc{gleichverteilt} im Dreieck $D$. \\
    ...
    \end{align*}
    Wenn $\zvx, Y$ unabhängig \txc{wären, dann würde gelten}:
    \txc{\begin{equation}
...
\komm{Symbole und Terme in Beispiel \autoref{bsp:dreieck}
    \begin{itemize}
        ...
        \item $f(x, y)$ = gemeinsame Dichte von $(\zvx, Y)$
        \item $f_X(x)$ beschreibt die \gls{verteilung} von $\zvx$ alleine
        \item $f_Y(y)$ beschreibt die \gls{verteilung} von $Y$ alleine
    \end{itemize}
}
...
Für $(x, y) \in [0,1]^2 \backslash D $ gilt $f(x, y) = 0$
    ...
    \]
    Denn wir haben ja oben ausgerechnet, dass $f_X (x) = 2 x  \chi_{[0,1]}(x)$ und $f_Y (y) = 2(1-y)\chi_{[0,1]}(y)$, INSOFERN $\zvx$ und $Y$ unabhängig sind. \\
