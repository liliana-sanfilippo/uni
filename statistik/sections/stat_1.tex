...
\begin{defi}\textbf{\gls{Streumaß}} \txc{(Skript 26)} \\
...
\begin{bsp}\textbf{Ein ganzer Zoo an \gls{Streumaß}en} \txc{(Skript 26)} \\
    ...
    Beweisen Sie als Hausaufgabe, dass es sich jeweils um \gls{\gls{Streumaß}}e handelt.
\end{bsp}
...
\begin{lem}\textbf{$p$-\gls{Quantilsabstand} als \gls{Streumaß}} \txc{(Skript 26)} \\
...
\begin{bsp}\textbf{Fragestellung im Fall von \gls{Bernoulli-Variable}n} \txc{(Skript 26)} \\
...
\begin{bsp}\textbf{\gls{Bernoulli-Variable}} \txc{(Skript 26)} \\
...
\begin{bsp}\textbf{\gls{Bernoulli-Variable}} \txc{(Skript 26)} \\
    Im Fall von unabhängigen \gls{Bernoulli-Variable}n als \gls{Stichprobenvariable} $X_1,\dots,X_n$ ist das zugehörige statistische Modell gegeben durch $\mathcal P = \{\pe_\theta:\theta\in\Theta\}$, wobei wir im Allgemeinen $\Theta=[0,1]$ wählen und $\pe_\theta$ durch die Wahrscheinlichkeitsfunktion $\wfk_\theta$ mit
    \[
...
Im Fall von \gls{Bernoulli-Variable}n heißt das, dass wir die Erfolgswahrscheinlichkeit $\theta$ schätzen wollen.
...
\begin{enumerate}
    \item \textbf{\gls{Punktschätzer}:} Resultat ist ein Wert $\theta$ oder $g(\theta)$ mit $\theta\in\Theta$
    \item \textbf{\gls{Bereichsschätzer}:} Resultat ist eine Teilmenge von $\Theta$
...
\begin{defi}\textbf{\gls{Punktschätzer}} \txc{(Skript 26)} \\
    Sei ein statistisches Modell der Form $\mathcal P=\{\pe_\theta:\theta\in\Theta\}$ gegeben.
    \begin{enumerate}[label=(\alph*)]
        \item Ein \emph{\gls{Schätzer}} für $\theta\in\Theta$ ist eine Abbildung $t:\mathcal X\to\Theta$.
        Wir nennen auch die Zufallsvariable $T=t(X)=t((X_1,\dots,X_n))$ einen \gls{Schätzer} für $\theta\in\Theta$.
        ...
        Ein \emph{\gls{Schätzer}} für $g(\theta)$ ist eine Abbildung $t:\mathcal X\to\Gamma$.
        Wir nennen auch die Zufallsvariable $T=t(X)=t((X_1,\dots,X_n))$ einen \gls{Schätzer} für $g(\theta)$.
        Eine Realisierung $t(x)=t((x_1,\dots,x_n))$ bzw.\ $T(\ele)$ heißt \emph{Schätzung} für $g(\theta)$.
    \end{enumerate}
    Wir werden uns zunächst auf \gls{Schätzer} für $\theta$ konzentrieren und später auf \gls{Schätzer} für $g(\theta)$ zurückkommen.
\end{defi}

\begin{bem}\textbf{Unterschied zwischen \gls{Schätzer} und Schätzung für $\theta$}
    \begin{mytable}
        \begin{tabular}{ll}
            \hline
...
\begin{bsp}\textbf{\gls{Schätzer} können absurd schlecht sein} \txc{(Skript 26)} \\
    Im Fall von unabhängigen \gls{Bernoulli-Variable}n als \gls{Stichprobenvariable} $X_1,\dots,X_n$ haben wir den \gls{Stichprobenraum} $\mathcal X=\{0,1\}^n$ sowie den Parameterraum $\Theta=[0,1]$.
    Gemäß Definition ist die Abbildung $t:\mathcal X\to\Theta$ mit $t(x)=\frac12$ für alle $x\in\mathcal X$ eine zulässige Wahl für einen \gls{Schätzer} der Erfolgswahrscheinlichkeit.
...
\begin{defi}\textbf{Kriterien zur Beurteilung von \gls{Punktschätzer}n} \txc{(Skript 26)} \\
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item Der \gls{Schätzer} $t$ heißt \emph{erwartungstreu}, wenn $\mathrm{Bias}_t(\theta)=0$ für alle $\theta\in\Theta$ gilt.
        \item Das \emph{Risiko} des \gls{Schätzer}s $t$ im Punkt $\theta\in\Theta$ ist definiert als die erwartete quadratische Abweichung des \gls{Schätzer}s von $g(\theta)$, das heißt,
        \[
            R(\theta,t) = \ET\big[(T-g(\theta))^2\big] \cdot
        \]
        \item Die \emph{Risikofunktion} des \gls{Schätzer}s $t$ ist definiert als die Abbildung $\theta\mapsto R(\theta,t)$.
        \item Sind $t_1,t_2:\mathcal X\to\Gamma$ zwei \gls{Schätzer} für $g(\theta)$, so sagen wir, dass $t_1$ \emph{mindestens so gut} ist wie $t_2$, wenn $R(\theta,t_1)\leq R(\theta,t_2)$ für alle $\theta\in\Theta$ gilt.
        Gilt zusätzlich, dass es ein $\theta\in\Theta$ gibt mit $R(\theta,t_1)<R(\theta,t_2)$, so sagen wir, dass $t_1$ \emph{besser} als $t_2$ ist.
    \end{enumerate}
\end{defi}

\begin{bsp}\textbf{Das arithmetische Mittel als \gls{Schätzer} im Fall von \gls{Bernoulli-Variable}n} \txc{(Skript 26)} \\
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \]
        weil für die \gls{Bernoulli-Variable}n $\ET[X_i]=\theta$ gilt.
        \item Daher ist der \gls{Schätzer} $t$ erwartungstreu.
        \item Das Risiko des \gls{Schätzer}s $t$ im Punkt $\theta\in\Theta$ ergibt sich zu
        \[
        ...
        \item Die Risikofunktion des \gls{Schätzer}s $t$ ist gegeben durch die Abbildung $\theta\mapsto\dfrac{\theta(1-\theta)}{n}$.
...
\begin{lem}\textbf{Rechenregeln für Bias und Risiko} \txc{(Skript 26)} \\
    Sei $t$ ein \gls{Schätzer} für $g(\theta)$.
    Dann gelten:
    \begin{enumerate}[label=(\alph*)]
        \item Der \gls{Schätzer} $t$ ist genau dann erwartungstreu, wenn $\ET[T]=g(\theta)$ für alle $\theta\in\Theta$ gilt.
...
\begin{bsp}\textbf{Das arithmetische Mittel als \gls{Schätzer} des Erwartungswerts} \txc{(Skript 26)} \\
...
\begin{bsp}\textbf{\gls{Schätzer} für die Varianz} \txc{(Skript 26)} \\
