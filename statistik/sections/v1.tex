...
Dann gelten:
    ...
    \paragraph{Beweis.}
    \begin{itemize}
        \item Untere Schranke: Da $\pe(A) = \sum_{\mu \in A} \mu(\omega)$ gilt und alle Summanden nichtnegativ sind,
        folgt $\pe(A) \geq 0$
        \item Obere Schranke: Aus
        \[
            \pe(A) = \sum_{\omega \in A} \mu(\omega) \leq \sum_{\omega \in \Omega} \mu(\omega) = 1
        \]
        ...
        \item $\pe(\Omega) = \sum_{\omega \in \Omega} \mu(\omega) = 1$ folgt direkt aus der Normierung der Wahrscheinlichkeitsfunktion.
    \end{itemize}
\end{lem}
...
\begin{defi}
    ...
    \begin{bem}
        ...
        Wahrscheinlichkeitsfunktion $\mu$ und dem Wahrscheinlichkeitsmaß $\pe$ gibt, bezeichnen wir auch das Paar $\wraum$
        als Wahrscheinlichkeitsraum.
        Die Zuordung von $\mu$ zu $\pe$ ist direkt durch die Definition von $\pe$ in Definition \autoref{def:zaehldichte} gegeben.
        Für eine
        gegebene Wahrscheinlichkeitsfunktion erhalten wir so ein eindeutig bestimmtes Wahrscheinlichkeitsmaß.
        Ist ein Wahrscheinlichkeitsmaß $\pe$ gegeben, so ergibt sich die zugehörige Wahrscheinlichkeitsfunktion $\mu$
        sofort aus den Wahrscheinlichkeiten der Elementarereignisse:
        \[
            \mu(\omega) = \pe(\{\omega\}), \faoio
        \]
        Beachten Sie hier, dass die Wahrscheinlichkeitsfunktion $\mu$ als Argument ein Elementarereignis, also
...
\begin{bem} % aus Skript
    ...
    Nehmen wir an, es gäbe eine Gleichverteilung $\glssymbol{wfk}$ auf einer abzählbar unendlichen Menge
    $\glssymbol{eraum}$.
    ...
    \[
        1 = \sum^{\infty}_{k = 1} \glssymbol{wfk}(k)    = c \cdot \# \mathbb{N}
    \]
