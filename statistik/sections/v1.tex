...
\begin{bem}
    \begin{itemize}
        ...
        \item Ist $\gslsymbol{elementarereignis} \in \glssymbol{eraum}$ ein Elementarereignis, so ist die
        einelementige Menge $\{\gslsymbol{elementarereignis}\} \in \glssymbol{potenz}(\glssymbol{eraum})$
        ein Ereignis in $\Omega$.
    \end{itemize}
\end{bem}
...
\begin{lem} \textbf{Eigenschaften des Wahrscheinlichkeitsmaßes für abzählbare Ereignisräume} \\
    Sei $\Omega \neq \emptyset$ eine abzählbare Menge und $\glssymbol{wmaß}$ ein Wahrscheinlichkeitsmaß auf $\Omega$.
Dann gelten:
    \begin{itemize}
        \item $0 \leq \pe (A) \leq 1$ für alle $A \leq \glssymbol{potenz}(\Omega)$
        \item $\pe(\epmtyset) = 0$
        \item $\pe(\Omega) = 1$
    \end{itemize}
