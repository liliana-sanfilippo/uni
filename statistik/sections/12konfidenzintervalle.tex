...
\begin{bsp}
    \txc{(2026)} \\
    Wenn die Stichprobenvariablen $X_1,\dots,X_n$ Poisson-verteilt sind mit Parameter $\lambda>0$, so gilt
    \[
        \lambda = \E{X_1} \cdot
    \]
    Wenn die Stichprobenvariablen $X_1,\dots,X_n$ exponentialverteilt sind mit Parameter $\alpha>0$, gilt dagegen
    \[
        \alpha = \frac{1}{\E{X_1}} \cdot
    \]
\end{bsp}

Es ist nicht immer möglich, den \gls{param} einer Verteilung aus dem EW zu berechnen.
...
\komm{
    ...
    einer Exponentialverteilung mit Parameter $\alpha=2$ illustriert (zwei unabhängige Durchläufe mit
...
\section{Übergang: die Normalverteilung}\label{sec:normalverteilung-uebergang}
...
zwei Parameter beschrieben wird.
...
\txc{(2026)} Die Beispiele zeigen, dass der geschätzte Parameter in der Regel nicht der wahre Parameter ist.

\paragraph{Idee.} Wir suchen ein Intervall, so dass wir mit vorgegebener \gls{ws} sagen können, dass der Parameter
...
\[
    ...
    1 - \frac\alpha2 \cdot
\]
Als Verteilungsfunktion ist $c\mapsto\Phi(c)$ monoton wachsend.
...
\begin{defi}
    \textbf{\gls{Konfidenzintervall}} \label{defi:konfidenzintervall} \txc{(2026)} \\
    ...
    unbekannten Parameter $\theta$ zum \textbf{Konfidenzniveau} $1-\alpha$.
\end{defi}
...
$t_{n-1,1-\alpha/2}$ das $(1-\alpha/2)$-Quantil der sogenannten \textbf{Studentschen $t$-Verteilung} mit $n-1$
Freiheitsgraden.

\begin{defi}
    \textbf{Studentsche $t$-Verteilung} \label{defi:studentsche-t-verteilung} \txc{(2026)} \\
    Die Studentsche $t$-Verteilung mit $n-1$ Freiheitsgraden ist die Verteilung der \gls{zv}
    \[
        T = \frac{\bar X - \wfk}{\widehat\sigma / \sqrt n} \cdot
    \]
    Diese \gls{zv} hat eine Dichte, und ihre Verteilungsfunktion ist gegeben durch das Integral über diese Dichte.
...
Wie bei der Normalverteilung sind wir für die Werte der Verteilungsfunktion auf eine Tabelle angewiesen.
\komm{
    Die Vorlesung zeigte hierzu Dichte-Plots der $t$-Verteilung für verschiedene Freiheitsgrade (u.\,a.\ 2, 3, 5,
    10, 30) im Vergleich zur Standardnormalverteilung: Für $n\to\infty$ nähert sich die $t$-Verteilung erkennbar
...
$t$-Verteilung nähert sich für große $n$ der Standardnormalverteilung an; siehe Bilder oben).
...
\komm{
    In den Hausaufgaben werden derartige Konfidenzintervalle (approximativ via $t$-Verteilung bzw.\ via
