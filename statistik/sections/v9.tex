...
\begin{satz}
    ...
    \]
    Dann gilt: \\
    \mab{\lim_{n \rightarrow \infty} \pe(\mid \frac{1}{n} S_n - \E{X_1} \mid \geq \epsilon) = 0 \quad  \forall \epsilon > 0}
...
\end{satz}
\komm{ Zu Satz \ref{satz:Moivre-Laplace} \\
...
\begin{bem}
    ...
    Die Stammfunktion \begin{equation}
                          \vfkt(y) = \int_{- \infty}^y \varphi(x) \dx \txc{ = \int_{- \infty}^y  \frac{1}{\sqrt{2\pi}} e^{x^2 / 2} \dx  }  \label{eq:equation4}
    \end{equation}
...
\begin{fr}
    ...
    \paragraph{1. Antwort:} $\phi (a) \approx \vfkt(-9) = 1 - \vfkt(9)$ und $\vfkt(9)$ ist so nah an 1, dass wir diesen
