...
\begin{bsp}
    ...
    \begin{align*}
        ...
        & \overset{\txc{*}}{\leq} \frac{1}{h^2} \Var(X)
    \end{align*}
...
\begin{bsp}
    ...
    % F 10 - 16
    Wie oft m¨ussen wir einen fairen W¨urfel werfen, bis mit Ws. $\geq 1  - 0.05$ die relative H¨aufigkeit einer Sechs um weniger als 1/10 von 1/6 abweicht?
    \paragraph{Ansatz:} $\eraum = \{1, ..., 6\}^N$ mit Gleichverteilung $\pe$ (f¨ur N W¨urfe) und Zufallsvariablen
    $X_i \eraum \rightarrow \{0,1\}$.

    Dabei sind $X_i(\ele) = 1_{\{6\}} (\ele_i)$ unabh¨angig und identisch verteilt (u.i.v.) mit $\pe(X_i = 1) = \frac{1}{6}$.

    $S_N = \sum^N_{i=1} X_i$ ist die Anzahl der geworfenen Sechsen und $\frac{1}{N} S_N$ ist die relative
    H¨aufigkeit einer Sechs.
    \paragraph{Gesucht} ist das kleinste N derart, dass $ \pe(| \frac{S_N}{N} - \frac{1}{6}| \geq \frac{1}{10}) \leq \frac{5}{100}$
    \txo{(Gegenereignis zu $\pe(| \frac{S_N}{N} - \frac{1}{6}| < \frac{1}{10}) \geq 1- \frac{5}{100}$ )}.
    ...
    und
    \[
        \Var(\frac{1}{N}  S_N) = \frac{1}{N} \Var(S_N) \overset{\txc{**}}{=} \frac{1}{N} Npq = \frac{1}{N} pq = \frac{1}{N} \frac{1}{6} \frac{5}{6}
    \]
    ...
    \[
        ...
        \leq 
        \frac{\Var(\frac{S_N}{N})}{(1/10)^2} = 100 \frac{5}{36 N}
    \]
    Wir w¨ahlen also N so groß, dass $100 \frac{5}{36 N} \leq \frac{5}{100}$.
    \paragraph{Fazit}
...
\begin{satz}
    ...
    %\texttt{Hier fehlen Voraussetzungen (siehe Folie 3 25)}
...
\begin{satz}
    ...
    \paragraph{Beweis.} \texttt{Folie 14--2025 bzw. F 25 in 2026}
    % TODO F 25
...
\begin{erg}
    ...
    \texttt{Siehe Folie 25--29 bzw. F 36--40}
    % TODO F 36-40
\end{erg}
\begin{bsp}
    ...
    Also: Die Anzahl der Erfolge \ma{S_n} ist \ma{\glssymbol{binomv}}-verteilt.
    ...
    \[
        A_n = \kl{\frac{3}{10}\leq \frac{1}{n} S_n  \leq \frac{7}{10}} = \kl{\lceil \frac{3}{10} \cdot n \rceil, \lceil  \frac{3}{10} \cdot n  \rceil + 1, \dots, \lfloor \frac{7}{10} \cdot n \rfloor}
    \]
