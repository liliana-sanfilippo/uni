...
\begin{satz}
    ...
    \]
    \paragraph{Beweis:} \txo{Spezialfall diskreter \gls{wraum}}
    \begin{align*}
...
\begin{satz}
    ...
    Grenzwertsatz.
    Approximieren der Summe mit Hilfe eines Riemann-Integrals.
...
\begin{bsp}
    ...
    Mit \ma{p = \frac{2}{3}}:
    \[
        \pe(A_n) = \sum_{k = \lceil \frac{3}{10} \cdot n \rceil}^{\lfloor \frac{7}{10} \cdot n \rfloor} \binom{142}{k} (\frac{2}{3})^k (\frac{1}{3})^{142-k} \txc{ = \sum_{k = 43}^{99} \binom{142}{k} (\frac{2}{3})^k (\frac{1}{3})^{142-k}}  \approx 0.80
    \]
...
\begin{fr}
    ...
    gewissen Sinne gegen $p = 2/3 \approx 66\%$ konvergiert. Daher ist die Forderung,
...
\begin{fr}
    ...
    möglich durch die so genannte Stetigkeitskorrektur: \index{Stetigkeitskorrektur}
    ...
    \end{align*}
    \txc{Bingo!} \\
    Beachte, daß die Stetigkeitskorrektur nicht immer besser ist (siehe Hausaufgaben).
\end{fr}
...
\begin{bsp}
    ...
    \paragraph{Was ist anders als im obigen Beispiel?} Neu an dieser Frage ist, daß wir nur eine untere Schranke gegeben haben,
...
\begin{bem}
    ...
    \[
        ...
        ) \approx 1 - \phi(\frac{c-np}{\sqrt{npq}})
    \]
    und
    \[
        \pe (s_n \leq d) = \pe(
        \frac{-1-np}{\sqrt{npq}} < \frac{S_n-np}{\sqrt{npq}} \leq \frac{d-np}{\sqrt{npq}}
        ) \approx \phi(\frac{d-np}{\sqrt{npq}})
    \]
    weil für große n
    \[
        \phi(\frac{-1-np}{\sqrt{npq}}) \approx 0 \quad und \quad \frac{n-np}{\sqrt{npq}} \approx 1
    \]
\end{bem}