...
\begin{bsp}[\gls{Schätzer} können absurd schlecht sein]
    \txc{(Skript 26)} \\
    Im Fall von unabhängigen \gls{Bernoulli-Variable}n als \gls{Stichprobenvariable} $X_1,\dots,X_n$ haben wir den \gls{Stichprobenraum} $\spr=\{0,1\}^n$ sowie den \gls{praum} $\praum=[0,1]$.
    ...
    Motiviert durch das Gesetz der großen Zahlen und die Tatsache, dass für \gls{Bernoulli-Variable} der Erwartungswert der Erfolgswahrscheinlichkeit entspricht, vermuten wir, dass das arithmetische Mittel der Stichprobenwerte
    \[
...
\begin{defi}[Kriterien zur Beurteilung von \gls{Punktschätzer}n]
    \txc{(Skript 26)} \\
    Gegeben seien ein statistisches Modell der Form $\pefam=\{\pe_{\param}:\param\in\praum\}$, eine Abbildung $g:\praum\to\Gamma$, sowie ein \gls{Schätzer} $\schatzer:\spr\to\Gamma$ für $g(\param)$.\footnote{Der Fall, dass wir mit einem \gls{Schätzer} für den \gls{param} $\param$ selbst arbeiten, entspricht der Wahl $\Gamma=\praum$ und $g(x)=x$ für alle $x\in\spr$. Wir müssen daher die beiden Fälle nicht unterscheiden.}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item Das \emph{Risiko} des \gls{Schätzer}s $\schatzer$ im Punkt $\param\in\praum$ ist definiert als die erwartete quadratische Abweichung des \gls{Schätzer}s von $g(\param)$, das heißt,
        \[
        ...
        \item Sind $\schatzer_1,t_2:\spr\to\Gamma$ zwei \gls{Schätzer} für $g(\param)$, so sagen wir, dass $\schatzer_1$ \emph{mindestens so gut} ist wie $\schatzer_2$, wenn $R(\param,t_1)\leq R(\param,t_2)$ für alle $\param\in\praum$ gilt.
        Gilt zusätzlich, dass es ein $\param\in\praum$ gibt mit $R(\param,t_1)<R(\param,t_2)$, so sagen wir, dass $\schatzer_1$ \emph{besser} als $\schatzer_2$ ist.
    \end{enumerate}
\end{defi}

\begin{bsp}
    \textbf{Das arithmetische Mittel als \gls{Schätzer} im Fall von \gls{Bernoulli-Variable}n} \txc{(Skript 26)} \\
    Im Fall von unabhängigen \gls{Bernoulli-Variable}n als \gls{Stichprobenvariable} $X_1,\dots,X_n$ erhalten wir folgende Ergebnisse für den \gls{Schätzer} $\schatzer$ mit
    \[
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \[
            R(\param,\schatzer) = \ET\left[\lef\schatzer(\frac1n\sum_{i=1}^n X_i - \param\righ\schatzer)^2\right] = \mathrm{Var}_\param\lef\schatzer(\frac1n\sum_{i=1}^n X_i\righ\schatzer) = \frac1n\,\mathrm{Var}_\param(X_1) = \frac{\param(1-\param)}{n} \cdot
        \]
        ...
        Das Risiko von $\tilde t$ in $\param$ ist gegeben durch $R(\param,\tilde \schatzer) = \dfrac{\param(1-\param)}{2}$ (dieselbe Rechnung wie für $\schatzer$, nur diesmal mit Stichprobengröße $2$).
...
\begin{bsp}
    ...
    Seien $X_1,\dots,X_n$ unabhängige \gls{Stichprobenvariable}\footnote{Wir nehmen nicht an, dass die Stichprobenvariablen \gls{Bernoulli-Variable} sind.} und $\schatzer$ ein \gls{Schätzer} für den Erwartungswert, der durch
    \[
        \schatzer(x) = \bar x = \frac1n\sum_{i=1}^n x_i \qquad \text{für alle } x\in\spr
    \]
    definiert ist.
    ...
    \[
        s^2(x) = \frac1n\sum_{i=1}^n (x_i-\bar x)^2 = \frac1n\sum_{i=1}^n \lef\schatzer(x_i - \frac1n\sum_{j=1}^n x_j\righ\schatzer)^2 \qquad \text{für alle } x\in\spr
    \]
    zu wählen.
    ...
    \begin{align*}
        X_i - \schatzer(X) &= (X_i-\ET[X_i]) + \lef\schatzer(\ET[X_1] - \frac1n\sum_{i=1}^n X_i\righ\schatzer) \\
        &= (X_i-\ET[X_i]) - \lef\schatzer(\frac1n\sum_{i=1}^n (X_i-\ET[X_i])\righ\schatzer) \\
    ...
    \[
        \ET\big[s^2(X)\big] = \lef\schatzer(1-\frac1n\righ\schatzer)\mathrm{Var}_\param(X_1) \cdot
    \]
