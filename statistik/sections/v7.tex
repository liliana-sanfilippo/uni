...
\begin{bsp}
    ...
    \[
        ...
        n\in \eraum = \nz.
    \]
    \txo{$\wfk(n) = (\frac{1}{2})^n = 2^{-n} \quad \froall n \in \eraum = \nz$} \\
...
\begin{satz}
    ...
    \[
        \E{Y} = \sum_{x \in X(\eruam)} f(x) \pe(X=x)
    \]
    \paragraph{Beweis.}
    \begin{align*}
        \E{Y} & =   \sum_{y \in Y(\eruam)} y \pe(Y=y) \\
        & =   \sum_{y \in Y(\eruam)} y  \sum_{x \in X(\eraum): f(x) = y\txo{*}} \pe(X=x) \\
        & =   \sum_{y \in Y(\eruam)}   \sum_{x \in X(\eraum): f(x) = y} \oub{y}{=f(x)} \pe(X=x) \\
        & =\sum_{y \in Y(\eruam)}   \sum_{x \in X(\eraum): f(x) = y} f(x) \pe(X=x) \\
