...
\begin{satz}
    ...
    \[
        \sum_{\oio} | aX(\ele) + bY(\ele)| \wfk(\ele) \leq |a| \oub{\sum_{\oio} |X(\ele)| \wfk(\ele)}{< \infty\ u.\ Vor.} + |b|  \oub{\sum_{\oio} |Y(\ele)| \wfk(\ele)}{< \infty\ u.\ Vor.} < \infty
    \]
...
\begin{bsp}
    ...
    \begin{itemize}
        ...
        \item Der Erwartungswert von \(X\) existiert, da der Bildbereich von \(X\)
        endlich ist.
    \end{itemize}

    \paragraph{1. Versuch} (mit binomischer Formel und etwas Geschick)
    \[
        \E{X} = \sum^n_{k=0} k \binom{n}{k} p^k (1-p)^{n-k} = ...
    \]
...
\begin{bsp}
    ...
    \[
        ...
        p=\frac12.
    \]
    Die Zufallsvariable $X:\eraum\to\rz$ sei definiert durch $ X(n)=2^n$ für alle $n\in\eraum$
