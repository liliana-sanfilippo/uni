...
\begin{defi}[\gls{Häufigkeitsverteilung}]
    ...
    \begin{enumerate}[label=(\alph*)]
        \item \textbf{Absolute Häufigkeit} \txc{= Anzahl} der Ausprägung $a_i$ in der Stichprobe $x_1,\dots,x_n$
        \txc{(Wie viele der Werte sind gleich $a$)}:
        \[
        ...
        \txc{(Gibt an, wie oft jeder beobachtete Wert in der Stichprobe vorkommt.)}
        \item \textbf{Relative Häufigkeit} \txc{= Anteil} der Ausprägung $a_i$ in der Stichprobe $x_1,\dots,x_n$:
        \[
            h_n(a_i) = \frac{1}{n} H_n(a_i)
        \]
        \txc{Gibt also an, welchen Anteil der Stichprobe ein bestimmter Wert ausmacht.}
...
\karkar{def_merkmal_stat}
\karkar{def_grundgesamtheit_stat_1}
\karkar{def_grundgesamtheit_stat_2}
\karkar{def_stichprobe_stat}
\karkar{def_urliste_stat}
\karkar{def_arten_von_merkmalen}
\karkar{def_arten_von_merkmalen_1}
\karkar{def_arten_von_merkmalen_2}
\karkar{def_absolute_haeufigkeit}
\karkar{def_relative_haeufigkeit}
%\karkar{bem_summe_haeufigkeiten}
%\karkar{bem_wahl_der_darstellung}
\karkar{bem_restklasse_bei_ausreissern}

\karkar{bem_uebersicht_lagemasse}
...
\begin{kom}[Andere Definitionen von Lagemaß]
    \textbf{Wikipedia:}  Ein Lagemaß oder Lageparameter ist in der Stochastik eine Kennzahl der Verteilung einer
    Zufallsvariable beziehungsweise
    ...
    \textbf{Björn Walther:} Lagemaße (auch Lageparameter) sind Kennzahlen, die
    ...
    \textbf{Welt der BWL:} Lagemaße der Statistik – auch Maße der zentralen Tendenz genannt – geben an, was in einer Datenmenge das Zentrum bzw. der Schwerpunkt ist, zum Beispiel der häufigste Wert oder der Durchschnitt.
\\
    % \href{https://studyflix.de/statistik/deskriptive-statistik-1052}{studyflix}

    \textbf{studyflix} Lageparameter geben die zentrale Tendenz des Datensatzes an.
...
\karkar{def_lagemass}
\karkar{bem_welche_mittel_sind_lagemasse}
\karkar{satz_schwerpunkteigenschaft_arith_mittel}
\karkar{satz_minimierungseigenschaft_arith_mittel}
\karkar{def_modalwert}
\karkar{bem_eigenschaften_modalwert}
\karkar{def_unimodal}
\karkar{def_median}
%\karkar{satz_median_ist_lagemass}
\karkar{satz_median_haelfte_eigenschaft}
\karkar{satz_median_charakterisierung}
\karkar{satz_median_minimierungseigenschaft}
...
\begin{kom}[Andere Definitionen von Streumaß]
    \textbf{Wikipedia:} Streuungsmaße, auch Dispersionsmaße (lateinisch dispersio „Zerstreuung“, von dispergere
    ...
    \textbf{Welt der BWL:} Streuungsmaße in der Statistik geben an, wie stark die einzelnen Datenwerte oder
    Messwerte streuen,
...
\begin{kom}[Andere Definitionen von empirisches $p$-Quantil]
    \textbf{Wikipedia:} Ein empirisches (p-)Quantil, auch Stichprobenquantil oder kurz Quantil (von lat. quantillus
    ...
    \textbf{studyflix:} Quantile gehören zu den Lagemaßen der Statistik.
    Sie teilen eine bestimmte Menge an Daten so ein, dass ein Teil p kleiner oder gleich und der andere Teil 1-p
    größer oder gleich dem Quantil ist. \\

    \textbf{Spektrum.de:} innerhalb der Mathematischen Statistik verwendeter Begriff für eine Größe, die eine
...
\karkar{bem_wozu_streumasse}
\karkar{def_streumass}
\karkar{def_spannweite}
\karkar{def_spannweite_1}
\karkar{def_medianabweichung}
\karkar{def_medianabweichung_1}
\karkar{bem_umgangssprachlich_medianabweichung}
\karkar{def_mittlere_absolute_abweichung_vom_median}
\karkar{def_mittlere_absolute_abweichung_vom_median_1}
\karkar{bem_umgangssprachlich_mittlere_abweichung_median}

\karkar{def_mittlere_absolute_abweichung_vom_mittel}
\karkar{def_mittlere_absolute_abweichung_vom_mittel_1}
\karkar{bem_umgangssprachlich_mittlere_abweichung_mittel}
\karkar{def_stichprobenvarianz}
\karkar{def_stichprobenvarianz_1}
\karkar{bem_umgangssprachlich_stichprobenvarianz}
\karkar{def_standardabweichung}
\karkar{bem_umgangssprachlich_standardabweichung}
\karkar{satz_steinersche_formel}
\karkar{satz_streumasse_via_haeufigkeitsverteilung}
\karkar{satz_streumasse_via_haeufigkeitsverteilung_1}

\karkar{def_empirisches_p_quantil}
\karkar{bem_umgangssprachlich_p_quantil}
\karkar{bem_quantil_eigenschaften}
\karkar{satz_charakterisierung_p_quantil}
\karkar{def_p_quantilsabstand}
\karkar{def_p_quantilsabstand_1}
\karkar{bem_umgangssprachlich_p_quantilsabstand}
\karkar{lem_p_quantilsabstand_ist_streumass}

\karkar{bem_uebersicht_robuste_streumasse}