...
\begin{bsp}[Beispiel Datenerhebung] \txc{(Skript 26)}
   \begin{mytable}
       \begin{tabular}{ll}
           \hline
...
\begin{bsp}[Schweinewürfeln]\txc{(Skript 26)} \\
...
\begin{bsp}[\gls{Merkmal}e treten in verschiedenen Ausprägungen auf] \txc{(Skript 26)}
    \begin{mytable}
...
\begin{defi}[Arten von Merkmalen]\txc{(Skript 26)} \\
...
\begin{defi}[\gls{Häufigkeitsverteilung}] \txc{(Skript 26)} \\
...
\begin{bsp}[Werfen von acht Schweinchen]\label{bsp:acht-schweinchen}  \txc{(Skript 26)} \\
...
\begin{bsp}[Alter von Berufstätigen] \txc{(Skript 26)}
    \[
        \begin{array}{l}
...
\begin{defi}[\gls{Lagemaß}]\label{def:lagemaß}  \txc{(Skript 26)} \\
...
\begin{kom}[Andere Definitionen von Lagemaß]
   ...
   \paragraph{\href{https://bjoernwalther.com/lagemasse-in-der-statistik/}{Björn Walther}:} Lagemaße (auch Lageparameter) sind Kennzahlen, die
   im Rahmen der deskriptiven Statistik genutzt werden.
    ...
    \paragraph{\href{https://welt-der-bwl.de/Lagema%C3%9Fe}{Welt der BWL}:} Lagemaße der Statistik – auch Maße der zentralen Tendenz genannt – geben an, was in einer Datenmenge das Zentrum bzw. der Schwerpunkt ist, zum Beispiel der häufigste Wert oder der Durchschnitt.

    \paragraph{\href{https://studyflix.de/statistik/deskriptive-statistik-1052}{studyflix}} Lageparameter geben die zentrale Tendenz des Datensatzes an.
...
\begin{bsp}[10$\times$ Würfeln]  \txc{(Skript 26)} \\
...
\begin{satz}[Eigenschaften des arithmetischen Mittels]  \txc{(Skript 26)} \\
\begin{enumerate}[label=(\alph*)]
...
\begin{defi}[Modalwert / Modus] \txc{(Skript 26)} \\
...
\begin{defi}[Unimodal] \txc{(Skript 26)} \\
...
\begin{bsp}[Würfeln]  \txc{(Skript 26)} \\
    \begin{figure}[H]
        \begin{subfigure}[t]{0.48\textwidth}
            \centering
            \begin{tikzpicture}
...
\begin{defi}[Median / Zentralwert] \txc{(Skript 26)} \\
    Sei $X$ ein quantitatives \gls{Merkmal}.
...
\begin{satz}[Eigenschaften des Medians]\label{satz:median_eigenschaften} \txc{(Skript 26)} \\
\begin{enumerate}[label=(\alph*)]
...
\begin{defi}[\gls{Streumaß}] \txc{(Skript 26)} \\
...
\begin{kom}[Andere Definitionen von Streumaß]
    ...
    \paragraph{\href{https://welt-der-bwl.de/Streuungsmaße}{Welt der BWL}:} Streuungsmaße in der Statistik geben an, wie stark die einzelnen Datenwerte oder Messwerte streuen,
...
\begin{kom}[Warum braucht man Streumaße?]
    ...
    (\href{https://www.destatis.de/DE/Service/Statistik-Campus/E-Learning/Modul16/streuungsmasse.html}{eLearning-Modul 16 - Streuungsmaße des statistischen Bundesamts})
    % https://www.destatis.de/DE/Service/Statistik-Campus/E-Learning/Modul16/streuungsmasse.html
\end{kom}

\begin{bsp}[Ein ganzer Zoo an \gls{Streumaß}en] \txc{(Skript 26)} \\
...
\begin{satz}[Berechnen der (Stichproben-)Varianz / Steinersche Formel]\label{satz:steinersche-formel}
\txc{(Skript 26)} \\
    \[
...
\begin{satz}[Berechnen von $\bar s$ und $s^2$ mittels der Häufigkeitsverteilung] \txc{(Skript 26)} \\
\begin{enumerate}[label=(\alph*)]
...
\begin{defi}[Empirisches $p$-Quantil] \txc{(Skript 26)} \\
    Sei $p\in(0,1)$.
    Dann heißt
    \[
        x_p =
...
\begin{satz}[Charakterisierung des $p$-Quantils] \txc{(Skript 26)}
\begin{enumerate}[label=(\alph*)]
...
\begin{defi}[$p$-Quantilsabstand] \txc{(Skript 26)} \\
    Sei $p\in(0,\tfrac12)$.
...
\begin{lem}[$p$-\gls{Quantilsabstand} als \gls{Streumaß}] \txc{(Skript 26)} \\
...
\begin{bsp}[Fragestellung im Fall von \gls{Bernoulli-Variable}n] \txc{(Skript 26)} \\
...
\begin{defi}[Stichprobe, Stichprobenvariable und Stichprobenraum] \txc{(Skript 26)}
\begin{enumerate}[label=(\alph*)]
...
\begin{bsp}[\gls{Bernoulli-Variable}]\txc{(Skript 26)} \\
...
\begin{defi}[Statistisches Modell] \txc{(Skript 26)} \\
...
\begin{defi}[\gls{Punktschätzer}] \txc{(Skript 26)} \\
...
\begin{bem}[Unterschied zwischen \gls{Schätzer} und Schätzung für $\theta$]
    \begin{mytable}
        \begin{tabular}{ll}
            \hline
...
\begin{bsp}[\gls{Schätzer} können absurd schlecht sein] \txc{(Skript 26)} \\
...
\begin{defi}[Kriterien zur Beurteilung von \gls{Punktschätzer}n] \txc{(Skript 26)} \\
