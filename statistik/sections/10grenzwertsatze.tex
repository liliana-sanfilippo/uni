...
\section*{Ergänzung zum Satz von de Moivre-Laplace}
...
\begin{er}
    ...
    \[
        \lim_{n \rightarrow \infty} \pe \left( a < \frac{\sn - np}{\sqrt{npq}} \leq b \right) = \frac{1}{\sqrt{2 \pi}} \intab e^{-x^{2} / 2} dx
    \]
    ...
    \[
        ...
        = 2 \vfkt \left(\frac{c}{\sqrt{npq}}\right) -1
    \]
    \txc{
   ...
   \begin{itemize}
       \item  Ist $c > 0$ nicht von n abhängig, so geht die \gls{ws} mit $n \rightarrow \infty$ gegen Null
       \txc{(c ist eine feste Konstante, hängt nicht von nn
       n ab)}
       \item Ist $ 0 < c \ll \sqrt{n}$ , so geht die \gls{ws} mit $n \rightarrow \infty$ immer noch gegen Null
       \txc{(c wächst, aber langsamer als $\sqrt{n}$)}
       \item Relevanter Beitrag, d.h., \gls{ws} größer als null, nur für $c \approx \sqrt{n}$ (oder größer)
...
\karkar{kor_moivre_laplace_symmetrisches_intervall}
\karkar{bem_wachstumsordnung_c_relevanz}
\karkar{bem_wann_normalapproximation_schlecht}

\section{Vergleich Gesetz der großen Zahlen vs. \gls{zgws}}\label{sec:vergleich-gesetz-der-groen-zahlen-vs.-zentraler-grenzwertsatz}
...
\begin{beo}
    ...
    \[
        \lim_{n \rightarrow \infty} \pe\left(\mid \frac{1}{n} \sn - \E{X_1} \mid \geq \epsilon\right) = 0 \quad \forall \epsilon > 0
    \]
    und Satz von de Moivre-Laplace
    \[
        \limnin \pe\left( a < \frac{\sn - np}{\sqrt{npq}} \leq b\right) = \frac{1}{\sqrt{2\pi}} \intab e^{x^2 / 2} \dx =
            \intab \varphi(x) \dx
    \]
    }
...
%n sein" oder "wie genau ist die Schätzung" oder "gib ein Konfidenzintervall an" → \gls{zgws} ist gefragt, mit der Φ\Phi
%Φ-Tabelle

\karkar{bem_ggz_vs_zgws_ordnung_epsilon}
\karkar{bem_klausur_faustregel_ggz_oder_zgws}
\karkar{bem_anwendungen_zgws_praxis}

\section{\gls{zgws}}\label{sec:zentraler-grenzwertsatz}

\begin{satz}
    \textbf{\gls{zgws}} \textit{Verallgemeinerung des Satzes von de Moivre-Laplace} \txc{(2026)}  \\
    Gegeben Zufallsvariablen $X_1, X_2, X_3, \dots$ auf einem Wahrscheinlichkeitsraum $\wraum$. \\
    Angenommen $X_1, X_2, X_3, \dots$ sind unabhängig und identisch verteilt und $\sigma^2 = \Var(X_i) \in (0, \infty)$ für alle i. \\
...
\begin{satz}
    \textbf{\gls{zgws} für \gls{Bernoulli-Variable}n} \txc{(2026)} \\
    Sind die Zufallsvariablen $X_1, X_2, X_3, \dots$ unabhängige \gls{Bernoulli-Variable}n, so
...
\begin{lem}
    ...
    \begin{itemize}
        \item Die Voraussetzung der identischen \gls{verteilung} kann abgeschwächt werden (z.B.\ zur Lindeberg-Bedingung).
        \begin{itemize}
            \item
            \txc{Also: man muss nicht verlangen, dass alle $X_i$ dieselbe \gls{verteilung} haben. Zum Beispiel darf
        ...
        \end{itemize}
        \item Die Voraussetzung der Unabhängigkeit kann abgeschwächt werden (\gls{zgws} von Lindeberg-Feller).
        \begin{itemize}
...
\begin{bsp}
    ...
    Wie groß ist die \gls{ws}, dass die Augensumme zwischen 1400 und 1550 liegt? \\
    \begin{itemize}
        \item $\eraum = \{1, \dots , 6\}^n$ mit $n = 420$
        \item $\pe$ Gleichverteilung auf $\eraum$
        \item $X_i(\ele) = \ele_i$ für $i = 1, \dots , n$ sind unabhängige, identisch verteilte ZV
...
\begin{bsp}
    ...
    \begin{figure}[H]
        ...
        \end{subfigure}\label{fig:figure11}
    \end{figure}
\end{bsp}

\karkar{satz_zentraler_grenzwertsatz}
\karkar{satz_zgws_bernoulli_spezialfall}
\karkar{lem_verallgemeinerungen_zgws}

\section{\gls{pgws}}\label{sec:poissonscher-grenzwertsatz}

\begin{satz}
    \textbf{\gls{pgws}} \textit{Approximationen der Binomialverteilung} \txc{(2026)} \\
    ...
    \]
    \paragraph{Beweis.} Sei k fest und  $np_n \rightarrow \alpha > 0$ für $ n \rightarrow \infty$.
    Daraus folgt
...
\begin{bem}
    Die Poisson-Approximation ist gut, wenn n groß im Vergleich zu k ist ($n \gg k$) und p klein ist (
    $n \gg \alpha \coloneqq np$).
    Das heißt insbesondere, dass die Erfolgswahrscheinlichkeit klein sein muss.
    % TODO für das untere auch karteikarte?
    Für derartige seltene Ereignisse verwenden wir $\pe (\sn = k) \approx \potm_{\alpha}(k)$ mit $\alpha \coloneqq np$.
\end{bem}
...
\begin{bsp}
    ...
    Wie groß ist die \gls{ws}, $ 0 \times, 1 \times, 2 \times, \dots$ zu gewinnen?
    \begin{mytable}
        \begin{tabular}{ccc}
...
\begin{bsp}
    ...
    Dies rechtfertigt, eine derartige \gls{verteilung} von Zellen oder Teilchen mit Hilfe der
    \gls{poisson_verteilung} zu modellieren.
\end{bsp}
...
\begin{bsp}
    ...
    \gls{ws}, dass ein Auftrag eintrifft in einem sehr kleinen
    Zeitfenster der Länge $\delta \ll 1$ sei näherungsweise $\alpha \delta$.
    ($\delta$ sehr klein, z.B.\ eine Millisekunde)). \\
    Wir zerlegen das Gesamtzeitintervall $[0, t)$ in N sehr kleine, disjunkte
    Teilintervalle der Länge $t/N$.
    (für ein festes t, z.B.\ 30 Minuten, 24 Stunden, drei Wochen, \ldots)
   \begin{center}
       $\left[0, t) = \bigcup_{k=1}^N [\frac{k-1}{N} \cdot t, \frac{k}{N} \cdot t \right)$
   \end{center}
    \gls{ws}, dass ein Auftrag eintrifft in einem bestimmten dieser Zeitfenster, ist
    näherungsweise $\alpha t / N$.
    \paragraph{Annahmen:} Für großes N sind die Zeitfenster so klein, dass wir annehmen dürfen, dass in
    jedem Fenster ein oder kein Auftrag eintrifft (d.h., wir schließen aus, dass mehr als ein Auftrag eintreffen kann)
    .
    ...
    \paragraph{Modellierung:} Damit haben wir jedem Zeitfenster, d.h.\ jedem k, eine \gls{Bernoulli-Variable}
    zugeordnet, die uns sagt, ob im zugehörigen Zeitfenster k ein Auftrag
    eintrifft.
    Die Erfolgswahrscheinlichkeit.
    dieser \gls{Bernoulli-Variable} ist $\alpha t / N$
    \txc{(Also die \gls{ws}, dass ein Auftrag in einem bestimmten Zeitfenster eintritt)}.
    ...
    \]
    Für die \gls{ws}, insgesamt $\varphi$ Aufträge im Intervall $[0, t)$ zu haben, ergibt die
    Poisson-Approximation für große N:
    \[
        \Bnmv_{N, \alpha t / N}(\varphi) \approx \potm_{N \cdot \alpha t / N}(\varphi) = \potm_{\alpha t}(\varphi)
    \]
    (Beachten Sie, dass wir in diesem Problem N beliebig groß machen dürfen.
    Wir verwenden
    hier die beliebige Teilbarkeit der Zeit!) \\
    Wiederum rechtfertigt unsere Betrachtung, beim Modellieren derartiger Prozesse
    generell die \gls{poisson_verteilung} zu verwenden.
\end{bsp}

% TODO Gemeinsamkeiten und unterschiede Normal- und Poisson-Approximation / Diskussion
...
\begin{lem}
    ...
    Tausche die Rollen von ``Erfolg'' und ``Misserfolg'' (zähle Misserfolge !)\\
    $\tilde\sn = n - \sn$ zählt die Anzahl der ``Misserfolge''  $\tilde\sn$ ist $\Bnmv_{n,q}$-verteilt. \\
    ...
    \]
\end{lem}
\subsection*{Beispielaufgabe Laubfrosch}\label{subsec:beispielaufgabe-laubfrosch} \txc{(2026)} \\
...
\begin{auf}
    ...
    \]
    Annahme: Frösche entwickeln sich unabhängig voneinander mit gleicher \gls{ws}
\end{auf}

\begin{auf}
    \subsubsection*{Zufallsvariable, die Anzahl der Frösche angbit, die \gls{verteilung} und ggf. Parameter}
    $\sn: \eraum \rightarrow \{ 0, \dots, n \}$ und
    \[
        \sn(\ele) \coloneqq \sumEN \ele_i \faoio
    \]
...
\begin{auf}
    ...
    und
    \[
        \Var(\sn) = np (1-p) = 5 \cdot \left(1 - \frac{1}{200}\right) = 5 \cdot \frac{199}{200} = \frac{199}{40}
    \]
\end{auf}
...
\karkar{satz_poissonscher_grenzwertsatz}
\karkar{bem_guete_poisson_approximation}
\karkar{bem_wann_poisson_approximation_gut}
\karkar{bem_allgemeines_modellierungsprinzip_poisson}
\karkar{lem_poisson_approximation_p_nahe_1}
\karkar{bem_keine_ausmultiplikation_bei_parametern}


\paragraph{Siehe auch}
\nameref{subsec:zusammenhang-poissonscher-grenzwertsatz-zentraler-grenzwertsatz-satz-von-de-moivre-laplace-und-gesetz-der-groen-zahlen} auf Seite~\pageref{subsec:zusammenhang-poissonscher-grenzwertsatz-zentraler-grenzwertsatz-satz-von-de-moivre-laplace-und-gesetz-der-groen-zahlen}.
