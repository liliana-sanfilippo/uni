...
\begin{defi}[\dichte einer \gls{zv}]
    ...
    \begin{equation}
        \label{eq:dichte-vf}
        \pex\bigl((-\infty, x]\bigr) = \pe(\zvx \leqslant x) = \int_{-\infty}^{x} f(u) \dd u
...
\begin{bsp}[Uniform verteilte \gls{zv}n]
    ...
    \[
        ...
        \begin{cases}
            0                 & \text{für } x < a ,\\
            \dfrac{x-a}{b-a}  & \text{für } a \leqslant x \leqslant b ,\\
            1                 & \text{für } x > b.
        \end{cases}
    \]
\end{bsp}
...
\begin{bsp}[Exponentialverteilte \gls{zv}n]
    ...
    \begin{itemize}
        \item Als erstes betrachten wir $x \leqslant 0$.
        Für alle $u \leqslant 0$ gilt $f(u) = 0$,
        woraus wieder
        \[
            \fxx(x) = \int_{-\infty}^{x} f(u) \dd u = 0
        \]
        folgt.
        \item Sei nun $x > 0$.
        ...
        \[
            \fxx(x) = \fxx(0) + \int_{0}^{x} \alpha \eu^{-\alpha u} \dd u
            = \int_{0}^{x} \alpha \eu^{-\alpha u} \dd u.
        \]
...
\begin{lem}[Rechnen mit \gls{verteilungsfunktion_zv}en II: \gls{zv} mit Dichte]
    ...
    \begin{enumerate}[label=(\alph*)]
        \item $\pe(a < \zvx \leqslant b) = \displaystyle\int_{a}^{b} f(x) \dd x = \fxx(b) - \fxx(a)$,
        \item $\pe(\zvx \leqslant b) = \displaystyle\int_{-\infty}^{b} f(x) \dd x = \fxx(b)$,
        \item $\pe(\zvx > a) = \displaystyle\int_{a}^{\infty} f(x) \dd x = 1 - \fxx(a)$,
        \item \begin{align*}
                  \pe(a \leqslant \zvx < b) = \pe(a < \zvx \leqslant b) = \pe(a \leqslant \zvx \leqslant b)
                  &= \pe(a < \zvx < b) = \fxx(b) - \fxx(a) ,\\
                  \pe(\zvx \leqslant b) &= \pe(\zvx < b) = \fxx(b) ,\\
...
\begin{defi}
    ...
    die Abbildung $\chi_B \colon \eraum \to \{0,1\}$, die gegeben ist durch
    \[
        \chi_B(\ele) =
        \begin{cases}
            1 & \text{für } \ele \in B ,\\
...
\begin{bsp}[Robin und Uli würfeln \dots]
    ...
    eine \emph{Eins} fällt, während Uli einen Euro gewinnt, wenn eine \emph{Sechs} fällt.
    ...
    \begin{align*}
        \zvx(\ele) &= \sum_{i=1}^{3} \chi_{\{1\}}(\ele_i) \quad \forall\, \ele \in \eraum
        && \text{(Robins Gewinn)}\\
        \zvy(\ele) &= \sum_{i=1}^{3} \chi_{\{6\}}(\ele_i) \quad \forall\, \ele \in \eraum
        && \text{(Ulis Gewinn)}
    \end{align*}
...
\begin{bsp}[Zufällige Punkte im Quadrat]
    ...
    berechnen:
    \begin{align*}
        |Q| &= \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} \chi_Q(x,y) \dd x \dd y
        = \int_{0}^{1} \left( \int_{0}^{1} 1 \dd x \right) \dd y = \int_{0}^{1} 1 \dd y = 1 ,\\
        |D| &= \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} \chi_D(x,y) \dd x \dd y
        = \int_{0}^{1} \left( \int_{y}^{1} 1 \dd x \right) \dd y
        = \int_{0}^{1} (1-y) \dd y = 1 - \frac12 = \frac12.
    \end{align*}
    ...
    muss $x \in [0,1]$ sowie $y \leqslant x$ erfüllen.
    Wir fassen diese beiden Bedingungen zusammen
    und erhalten ein Integral der Form $\int_y^1 \dots \dd x$.
    ...
    \[
        |D| = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} \chi_D(x,y) \dd x \dd y
        = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} \chi_D(x,y) \dd y \dd x
        = \int_{0}^{1} \left( \int_{0}^{x} 1 \dd y \right) \dd x
        = \int_{0}^{1} x \dd x = \frac12.
    \]
...
\begin{defi}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \[
            \pe(\zvx_1 \leqslant a_1, \dots, \zvx_n \leqslant a_n)
        ...
        gilt.
    \end{enumerate}
    Dabei verstehen wir unter $\{\zvx_1 \leqslant a_1, \dots, \zvx_n \leqslant a_n\}$ das Ereignis, dass
    alle $n$ Bedingungen simultan erfüllt sind, das heißt,
    \begin{align*}
        \{\zvx_1 \leqslant a_1, \dots, \zvx_n \leqslant a_n\}
        &= \{\ele \in \eraum \colon \zvx_1(\ele) \leqslant a_1 \text{ und } \dots \text{ und }
        \zvx_n(\ele) \leqslant a_n\}\\
        &= \{\zvx_1 \leqslant a_1\} \cap \dots \cap \{\zvx_n \leqslant a_n\}\\
        &= \bigcap_{i=1}^{n} \{\zvx_i \leqslant a_i\}.
    \end{align*}
\end{defi}
...
\begin{defi}
    ...
    \[
        f(x_1, \dots, x_n) = \frac{1}{|C|} \chi_C\bigl((x_1, \dots, x_n)\bigr) =
        \begin{cases}
...
\begin{lem}[Berechnen von Randdichten]
    ...
    \[
        ...
        \dd x_1 \dots \dd x_{k-1} \dd x_{k+1} \dots \dd x_n \qquad \forall\, x \in \mathbb{R} ,
    \]
    ...
    \begin{proof}
        ...
        \begin{align*}
            \pe(\zvx \leqslant a) &= \pe(\zvx \leqslant a, \zvy \in \mathbb{R})\\
            &= \int_{-\infty}^{\infty} \int_{-\infty}^{\infty}
            f(x,y) \chi_{(-\infty,a]}(x) \dd x \dd y\\
            &= \int_{-\infty}^{\infty} \int_{-\infty}^{a} f(x,y) \dd x \dd y.
        \end{align*}
        Da $f$ eine nichtnegative, messbare Funktion ist, dürfen wir die Integrationsreihenfolge
        vertauschen.
        Damit folgt
        \[
            \pe(\zvx \leqslant a) = \int_{-\infty}^{a} \left( \int_{-\infty}^{\infty} f(x,y) \dd y
            \right) \dd x = \int_{-\infty}^{a} f_\zvx(x) \dd x
        \]
        mit $f_\zvx(x) = \int_{-\infty}^{\infty} f(x,y) \dd y$.
...
\begin{defi}
    ...
    Die Dichte $f_k$ von $\zvx_k$ gemäß Lemma 3.35 nennen wir \emph{$k$-te Randdichte von
        $(\zvx_1, \dots, \zvx_n)$}.
\end{defi}
