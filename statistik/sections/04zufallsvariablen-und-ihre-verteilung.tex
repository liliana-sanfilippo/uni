...
\begin{itemize}
    \item Konzepte
    \begin{itemize}
        \item Zufallsvariablen als Abbildung und ihre Verteilung als \gls{wmaß}
...
\begin{defi}
    ...
    Sei $\zvx : \eraum \rightarrow E$ eine E-wertige Zufallsvariable auf einem \gls{wraum} $\wraum$.
    Dann
...
\begin{defi}[Verteilung einer \gls{zv}]
    ...
    \]
    Das \gls{wmaß} $\pex$ auf $E$ heißt Verteilung von $\zvx$.
\end{defi}
...
\begin{bsp}[Fortsetzung Münzwurf]
    ...
    Die Zufallsvariable $\sn$ aus Beispiel \autoref{bsp:n-faches-Werfen-einer-fairen-Münze} hat den Bildbereich $\sn(\eraum_n)  = \{0, \dots , n\}$.
...
\begin{bem}[Sprunghöhen der Verteilungsfunktion]
...
\begin{bsp}
    ...
    Zufallsvariable ausgedrückt werden, nicht immer wieder mühsam $\{\ele \in \eraum : \zvx(\ele) \in \dots \}$ schreiben
    müssen.
\end{bsp}
...
\begin{lem}[Existenz einer Zufallsvariablen mit vorgegebener Verteilung)]
    ...
    derart, dass die Verteilung von $\zvx$ das vorgegebene \gls{wmaß} $\tilde\pe$ ist.
...
\komm{Zu jedem \gls{wmaß} auf einem Messraum existiert eine Zufallsvariable, die genau dieses Maß als Verteilung hat. \\
...
\begin{bem}[Sprunghöhen der Verteilungsfunktion]
    ...
    Am Beispiel der geometrischen Verteilung sehen wir in Abbildung 3.2 \txc{(siehe Skript 2026)}, dass die Sprunghöhen
    der Verteilungsfunktion den Werten der Wahrscheinlichkeitsfunktion $\wfk_{\zvx}$ entsprechen. Das ist kein Zufall.
    Nimmt
    eine Zufallsvariable $\zvx$ nur abzählbar viele Werte $x_1, x_2, . . .$ an, so gilt stets
    \begin{align*}
...
Wir ergänzen nun unser Lemma über die Eigenschaften von Verteilungsfunktionen um eine Aussage
über diskrete Zufallsvariablen.

\begin{lem}[Eigenschaften von Verteilungsfunktionen II: diskrete Zufallsvariable]
    ...
    \begin{itemize}
        ...
        \item $\fxx$ ist rechtsstetig, d.h.: Für jedes $x_0 \in \rz$ und jede Folge $(x_n)_{n \in \nz}$ mit $x_n \searrow x_0$ gilt $\limnin \fxx (x_n) = \fxx (x_0)$
        \item $\lim_{x \rightarrow - \infty} \fxx =(x) = 0$ und $\limnin \fxx (x) = 1$
        \item Nimmt die Zufallsvariable $\zvx$ nur abzählbare viele Werte an, so ist $\fxx$ stückweise konstant.
    \end{itemize}
\end{lem}


\section{Zufallsvariablen mit Dichte}\label{sec:zufallsvariablen-mit-dichte}
\begin{defi}[\dichte einer \gls{zv}] \txc{(Skript 26 - Definition 3.21.)}  \\
todo  % TODO
\end{defi}
...
\begin{bem}
    ...
    todo  % TODO
    % Umgekehrt können wir natürlich Gleichung 3.1 verwenden, um zu einer gegebenen Dichte f eine Zufallsvariable X zu
    %konstruieren, deren Verteilung PX die Dichte f hat
\end{bem}
